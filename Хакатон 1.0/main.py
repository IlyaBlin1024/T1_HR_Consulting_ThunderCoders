...
SCIBOX_API_KEY = os.getenv("sk-Kk1N_G-MJLcV2pEDgN2URg", "").strip()     # читаем ключ Scibox (пустая строка, если не задан)
SCIBOX_BASE_URL = os.getenv("https://llm.t1v.scibox.tech/v1")  # URL Scibox по умолчанию

# ============================== ИНИЦИАЛИЗАЦИЯ ПРИЛОЖЕНИЯ =======================
app = FastAPI(title="Career Backend", version="1.2.2")       # создаём экземпляр FastAPI с метаданными

app.add_middleware(                                          # подключаем CORS-мидлварь
...
def scibox_client() -> Optional[OpenAI]:                     # создаём клиента Scibox (если есть ключ)
    if not SCIBOX_API_KEY:                                   # если ключ не задан
        return None                                          # возвращаем None — будем работать в оффлайн-режиме
    return OpenAI(api_key=SCIBOX_API_KEY, base_url=SCIBOX_BASE_URL)  # создаём клиента с кастомным base_url
...
def node_llm_reply(state: ChatState) -> Dict[str, Any]:        # узел 3: генерация ответа LLM
    ...
    course_lines = "\n".join([                                  # человекочитаемый список курсов
        f"- {c['title']} ({c['provider']}) — фокус: {', '.join(c['skills'])}" for c in courses
    ])
    base_prompt = (                                            # формируем промпт для модели
    ...
    if not client:                                             # если ключа нет или клиент не создан
        return {"llm_reply": "Курсы подобраны. Начните с №1, затем №2. Пробелы: KPI, риски, коммуникации. План на 2 недели: выполнить вводные модули, описать 3 KPI, оформить risk-log, подготовить апдейт стейкхолдерам, пройти практикум и внедрить метрики."}  # оффлайн-ответ
    try:                                                       # пробуем вызвать модель
        resp = client.chat.completions.create(                 # вызов chat.completions Scibox
            model="Qwen2.5-72B-Instruct-AWQ",                 # модель
            messages=[{"role": "user", "content": base_prompt}],  # сообщения в чат
            temperature=0.3, top_p=0.9, max_tokens=700        # параметры генерации
        )
        return {"llm_reply": resp.choices[0].message.content} # возвращаем текст ответа ассистента
    except Exception:                                          # если запрос упал
        return {"llm_reply": "Не удалось получить ответ от LLM. Используйте предложенную подборку курсов и начните с самого релевантного."}  # безопасный фолбэк
